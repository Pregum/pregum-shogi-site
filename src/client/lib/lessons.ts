...
const tarefu: Lesson = {
  ...
  steps: (() => {
    ...
    const steps = seq(start, '2筋の歩がお互いいなくなった局面。持ち歩1枚の使いどころです。', [
      ...
      { move: mv(1, 3, 1, 4), caption: '後手はこの歩を直接取る駒がありません。' },
      {
...
const jujiBisha: Lesson = {
  ...
  steps: seq(
    ...
    '後手の銀(5三)と角(7五)。バラバラに見えますが、5五の地点に注目してください。',
    [
      {
        move: mv(5, 8, 5, 5),
        caption:
          '▲5五飛！ 縦(5三の銀)と横(7五の角)を同時ににらむ「十字飛車」。どちらかは必ず取れます。',
      },
      { move: mv(7, 5, 8, 4), caption: '後手は大事な角を逃がしましたが…' },
      {
        move: mv(5, 5, 5, 3, true),
        caption:
          '▲5三飛成。銀を取ったうえに龍まで作れて大戦果。飛車を使うときは、縦だけでなく「横の利き」も常に意識——それが十字飛車の心です。',
      },
    ],
  ),
};
