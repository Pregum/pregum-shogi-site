    ...
    const steps = seq(start, '後手玉は2一。龍は近くにいますが王手はかかっていません。ここで決め手があります。', [
      {
        ...
        caption:
          '▲3一銀打！ 玉の腹(真横)に銀。王手ではありませんが、玉はどこへも動けません(1二・2二・3二は龍の横利き、3一の銀は龍が守っていて取れない)。',
      },
      {
        move: { to: sq(3, 2), drop: 'FU' },
        caption: '後手は△3二歩と、龍の横利きを遮って脱出路を作ろうとしますが——',
      },
      {
        move: { to: sq(2, 2), drop: 'KI' },
        caption: '▲2二金打！ 3一の銀が金を支えていて、これで詰みです。',
      },
    ]);
    steps.push({
      pos: steps[steps.length - 1].pos,
      caption:
        '1二は金の横利き、3二は自分の歩、1一は自分の香が邪魔。金も銀も龍がにらんでいて取れません。「王手は追う手、腹銀は縛る手」——王手をかけない一手が一番厳しいこともあるのです。',
    });
    return steps;
  })(),
};
...
const hisshi: Lesson = {
  ...
  steps: (() => {
    ...
    const steps = seq(start, '後手玉は1一。持ち駒は金1枚。王手をかけたくなりますが、もっと強い手があります。', [
      {
        move: { to: sq(1, 3), drop: 'KI' },
        caption:
          '▲1三金打！ 王手ではありません。しかし次の▲1二金(龍が支える頭金)が絶対に受からない——これが「必至」です。玉は1二も2一も2二も龍と金の利きで動けません。',
      },
      {
        move: { to: sq(2, 2), drop: 'FU' },
        caption: '後手は△2二歩と、龍の縦利きを遮る懸命の受け。2一に逃げ道ができたように見えますが——',
      },
      {
        move: mv(2, 3, 2, 2),
        caption: '▲同龍！ 受けの歩ごと食い破ります。1三の金が龍を支えていて、これで詰みです。',
      },
    ]);
    steps.push({
      pos: steps[steps.length - 1].pos,
      caption:
        'どんな合駒を打っても▲同龍、何もしなくても▲1二金。受けが1つも存在しない状態が「必至」です(単なる「詰めろ」は受けが残っている)。終盤は王手の連続より、必至をかけるほうが確実に勝てます。',
    });
    return steps;
  })(),
};
